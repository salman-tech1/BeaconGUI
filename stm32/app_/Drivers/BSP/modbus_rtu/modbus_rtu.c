...
static Modbus_Status_t Modbus_GPIO_UART_Init(uint32_t baudrate)
{
    ...
    if (HAL_UARTEx_SetTxFifoThreshold(&s_modbus_uart_handle, UART_TXFIFO_THRESHOLD_1_8) != HAL_OK) {
            return MODBUS_ERROR;
        }
        if (HAL_UARTEx_SetRxFifoThreshold(&s_modbus_uart_handle, UART_RXFIFO_THRESHOLD_1_8) != HAL_OK) {
            return MODBUS_ERROR;
        }
        if (HAL_UARTEx_DisableFifoMode(&s_modbus_uart_handle) != HAL_OK) {
            return MODBUS_ERROR;
        }
