...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
static void modbus_test_task(void *pv)
{
    (void)pv;

    if (Modbus_RTU_Init(9600) != MODBUS_OK) {
        Log_Printf(LOG_LEVEL_ERROR, "ModbusTest", "Init failed");
        vTaskDelete(NULL);
    }
    ...
    for (;;)
    {
        Modbus_Status_t st = Modbus_RTU_ReadHoldingRegisters(1, 0x0000, 2, regs);

        if (st == MODBUS_OK) {
            Log_Printf(LOG_LEVEL_INFO, "ModbusTest",
                       "reg[0]=%u reg[1]=%u", regs[0], regs[1]);
        } else {
            Log_Printf(LOG_LEVEL_WARN, "ModbusTest", "read failed, status=%d", (int)st);
        }
