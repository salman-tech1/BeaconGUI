 ...
 *
 *  turns Com's raw bytes into framed, CRC-verified
 *  packets, and turns outgoing packets back into raw bytes for Com.
 */
...
/* Command IDs. Link doesn't interpret these — it just carries them.
   Meaning/handling is entirely an App-layer (link_service) concern. */
 ...
 * @note   Runs in whatever context calls frame_Poll() — a task, never an
 *         ISR (frame_Poll itself must never be called from ISR context).
 *         Keep implementations fast: copy and return.
 */
 ...
 * @note   NOT reentrant (uses a single static frame buffer). If more than
 *         one task can call this, serialize access yourself — see
 *         link_Send() in the App layer for a ready-made wrapper.
 */
frame_Status_t frame_Send(uint8_t cmd, uint16_t seq, uint8_t flags,
                         const uint8_t *payload, uint16_t payload_length);

/**
 * @brief  Drain whatever new bytes Com has buffered and feed them through
 *         the parser. Call this once per RX wake-up, from task context.
 */
void frame_Poll(void);

/** CRC-16/IBM. MUST be byte-for-byte identical to the ESP32 implementation. */
